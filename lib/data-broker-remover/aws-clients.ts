...
export function getSESClient(): SESClient {
  return new SESClient({
    region:
      process.env.AWS_REGION || process.env.VITE_AWS_REGION || "eu-west-2",
  });
}
...
export function getVerificationFromEmail(): string {
  return process.env.SES_FROM_EMAIL || "noreply@visiblelabs.org";
}

export function getRequestFromEmail(): string {
  return process.env.SES_REQUESTS_EMAIL || "requests@visiblelabs.org";
}
